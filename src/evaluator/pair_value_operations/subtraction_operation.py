...
class SubtractionOperation(PairValuesOperation):
    ...
    def calculate(self, first: str, second: str) -> str:
        ...
        if not (isinstance(parsed_first, float) and isinstance(parsed_second, float)):
            raise EvaluationError("SubtractionSupportNumbersOnly")
        try:
            return str(parsed_first - parsed_second)
        except Exception as e:
            raise EvaluationError(f"FailedSubtracting {first} - {second}") from e
    ...
    def documentation(self) -> str:
        ...
        @returns: first number minus the second
        """
